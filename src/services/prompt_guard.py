...
class PromptGuard:
    def __init__(self, extra_patterns: list[tuple[str, str, str]] | None = None):
        ...
        self._compiled = [(re.compile(p, re.IGNORECASE), sev, name) for p, sev, name in self._patterns]
    ...
    def scan(self, text: str) -> InjectionScanResult:
        ...
        if threats:
            logger.warning("Prompt injection detected", threat_count=len(threats),
                          high_severity=not is_safe)
